...
### Then, I need to draw the OECD graph only
oecd_countries <- c(
...
ggsave("scatter_lgdp_spi_2025_oecd.png", p_scatter, width = 24, height = 18, dpi = 300, limitsize = FALSE)

# 5) Linear regression (OECD only)
p_with_lm <- p_scatter +
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.9, fullrange = FALSE)

# Run regression on OECD sample
model <- lm(spi ~ lgdp, data = df_oecd)
sm <- summary(model)
...
# Auto position near top-left of the OECD data range
xr <- range(df_oecd$lgdp, na.rm = TRUE)
yr <- range(df_oecd$spi, na.rm = TRUE)
x_pos <- xr[1] + 0.05 * diff(xr)   # 5% in from left
y_pos <- yr[2] - 0.05 * diff(yr)   # 5% down from top
