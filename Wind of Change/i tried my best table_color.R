...
# ---- Packages ----
required <- c("tidyverse", "cowplot", "haven")
...
df <- read_dta("PUBLIC_DATA/COUNTRY_LEVEL_PUBLIC.dta") %>%
  zap_labels()
...
# ---- Step 4: Plot function (vertical style, horizontal CIs) ----
make_plot <- function(m) {
  pd <- effects_df %>%
    ...
    mutate(
      ...
      group = case_when(
        interaction == "Above mean GDP 1850" ~ ifelse(group_flag == 1, "Above mean", "Below mean"),
        interaction == "Above top 25% GDP 1850" ~ ifelse(group_flag == 1, "Top 25%", "Bottom 75%"),
        TRUE ~ "All"
      )
    )
  
  ggplot(pd, aes(x = effect, y = country, color = group)) +
    geom_point(size = 2) +
...
for (m in 1:8) {
  ...
  ggsave(fname, plot = p, width = 9, height = 14, dpi = 300)  # taller per plot
}

# ---- Step 6: Combine into 4x2 panel ----
...
ggsave("Table8_country_effects_models1-8.png",
       plot = final_plot, width = 20, height = 40, dpi = 300)

message("✅ Saved 8 individual plots + 4x2 combined panel plot, vertical style with group coloring")
